...
import { wrapStackit } from './stackit.js';
...
describe('wrapStackit', () => {
  it('tracks usage when completion includes usage data', async () => {
    ...
    const tracked = wrapStackit(client as never, tracker as never);
  ...
  it('uses llmeter_customer_id from options when provided', async () => {
    ...
    await tracked.chat.completions.create(
      ...
      { llmeter_customer_id: 'german_tenant_xyz' } as never
    );

    expect(tracker.track).toHaveBeenCalledWith(
      expect.objectContaining({ customerId: 'german_tenant_xyz' })
    );
  });
  ...
  it('uses defaultCustomerId when llmeter_customer_id not set', async () => {
    ...
    const tracker = makeTracker();
    const tracked = wrapStackit(client as never, tracker as never, 'schwarz_enterprise');

    await tracked.chat.completions.create({ model: 'mixtral-8x7b-instruct', messages: [] });

    expect(tracker.track).toHaveBeenCalledWith(
      expect.objectContaining({ customerId: 'schwarz_enterprise' })
    );
  });
