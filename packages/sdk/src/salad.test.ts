...
import { wrapSalad } from './salad.js';
...
describe('wrapSalad', () => {
  it('tracks usage when completion includes usage data', async () => {
    ...
    const tracked = wrapSalad(client as never, tracker as never);
  ...
  it('uses llmeter_customer_id from options when provided', async () => {
    ...
    await tracked.chat.completions.create(
      ...
      { llmeter_customer_id: 'salad_customer_123' } as never
    );

    expect(tracker.track).toHaveBeenCalledWith(
      expect.objectContaining({ customerId: 'salad_customer_123' })
    );
  });
  ...
  it('removes llmeter_customer_id from options before forwarding', async () => {
    ...
    await tracked.chat.completions.create(
      ...
      { llmeter_customer_id: 'denver_dev_456', stream: false } as never
    );
  ...
  it('uses defaultCustomerId when llmeter_customer_id not set', async () => {
    ...
    const tracker = makeTracker();
    const tracked = wrapSalad(client as never, tracker as never, 'salad_app');

    await tracked.chat.completions.create({ model: 'llama-3.1-8b-instruct', messages: [] });

    expect(tracker.track).toHaveBeenCalledWith(
      expect.objectContaining({ customerId: 'salad_app' })
    );
  });

  it('passes through non-chat properties unchanged', async () => {
    const client = {
      ...makeMockClient({ model: 'phi-3.5-mini-instruct' }),
      models: { list: vi.fn() },
    };
    const tracker = makeTracker();
    const tracked = wrapSalad(client as never, tracker as never);
  ...
  it('forwards params correctly to underlying client', async () => {
    ...
    const params = {
      model: 'gemma-2-9b-it',
      messages: [{ role: 'user', content: 'Hello from a gaming PC in Denver!' }],
      temperature: 0.7,
    };
