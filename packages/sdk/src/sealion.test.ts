...
import { wrapSeaLion } from './sealion.js';
...
describe('wrapSeaLion', () => {
  it('tracks usage when completion includes usage data', async () => {
    ...
    const tracked = wrapSeaLion(client as never, tracker as never);
  ...
  it('uses llmeter_customer_id from options when provided', async () => {
    ...
    await tracked.chat.completions.create(
      { model: 'sea-lion-7b-instruct', messages: [] },
      { llmeter_customer_id: 'sealion_user_456' } as never
    );

    expect(tracker.track).toHaveBeenCalledWith(
      expect.objectContaining({ customerId: 'sealion_user_456' })
    );
  });
  ...
  it('strips llmeter_customer_id from options passed to the provider', async () => {
    ...
    await tracked.chat.completions.create(
      { model: 'gemma-sea-lion-9b-it', messages: [] },
      { llmeter_customer_id: 'sealion_enterprise_789', stream: false } as never
    );
  ...
  it('falls back to defaultCustomerId when no llmeter_customer_id in options', async () => {
    const client = makeMockClient({
      model: 'sea-lionv2.1-8b-instruct',
      usage: { prompt_tokens: 150, completion_tokens: 75 },
    });
    const tracker = makeTracker();
    const tracked = wrapSeaLion(
      client as never,
      tracker as never,
      'default-sealion-tenant'
    );
    ...
    expect(tracker.track).toHaveBeenCalledWith(
      expect.objectContaining({ customerId: 'default-sealion-tenant' })
    );
  });

  it('proxies all other properties on the client', async () => {
    const client = {
      ...makeMockClient({ model: 'deepseek-v3' }),
  ...
  it('returns the completion result unchanged', async () => {
    const response = {
      ...
      choices: [{ message: { role: 'assistant', content: 'Saya boleh membantu anda dalam Bahasa Melayu.' } }],
    };
    ...
    const result = await tracked.chat.completions.create({
      model: 'sea-lionv3-8b-instruct',
      messages: [{ role: 'user', content: 'Bolehkah awak berbahasa Melayu?' }],
    });
