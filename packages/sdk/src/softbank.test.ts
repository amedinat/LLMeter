...
import { wrapSoftBank } from './softbank.js';
...
describe('wrapSoftBank', () => {
  it('tracks usage when completion includes usage data', async () => {
    ...
    const tracked = wrapSoftBank(client as never, tracker as never);
  ...
  it('uses llmeter_customer_id from options when provided', async () => {
    ...
    await tracked.chat.completions.create(
      { model: 'sarashina2-7b', messages: [] },
      { llmeter_customer_id: 'softbank_user_456' } as never
    );

    expect(tracker.track).toHaveBeenCalledWith(
      expect.objectContaining({ customerId: 'softbank_user_456' })
    );
  });

  it('does not track when usage is missing', async () => {
    const client = makeMockClient({ model: 'meta-llama/Llama-3.1-70B-Instruct' });
    const tracker = makeTracker();
    const tracked = wrapSoftBank(client as never, tracker as never);
  ...
  it('strips llmeter_customer_id from options passed to the provider', async () => {
    ...
    await tracked.chat.completions.create(
      { model: 'sarashina2-13b', messages: [] },
      { llmeter_customer_id: 'softbank_enterprise_789', stream: false } as never
    );
  ...
  it('falls back to defaultCustomerId when no llmeter_customer_id in options', async () => {
    ...
    const tracker = makeTracker();
    const tracked = wrapSoftBank(
      client as never,
      tracker as never,
      'default-softbank-tenant'
    );
    ...
    expect(tracker.track).toHaveBeenCalledWith(
      expect.objectContaining({ customerId: 'default-softbank-tenant' })
    );
  });
  ...
  it('returns the completion result unchanged', async () => {
    const response = {
      ...
      choices: [{ message: { role: 'assistant', content: '日本のAI技術は急速に発展しています。' } }],
    };
