...
import { wrapHitachi } from './hitachi.js';
...
describe('wrapHitachi', () => {
  it('tracks usage when completion includes usage data', async () => {
    ...
    const tracked = wrapHitachi(client as never, tracker as never);
  ...
  it('uses llmeter_customer_id from options when provided', async () => {
    ...
    await tracked.chat.completions.create(
      { model: 'hai-7b-instruct', messages: [] },
      { llmeter_customer_id: 'hitachi_user_456' } as never
    );

    expect(tracker.track).toHaveBeenCalledWith(
      expect.objectContaining({ customerId: 'hitachi_user_456' })
    );
  });
  ...
  it('passes through the original result', async () => {
    const response = {
      ...
      choices: [{ message: { content: '日立のLumadaプラットフォームについてお答えします。' } }],
    };
  ...
  it('uses defaultCustomerId when no llmeter_customer_id provided', async () => {
    ...
    const tracker = makeTracker();
    const tracked = wrapHitachi(client as never, tracker as never, 'hitachi_default');

    await tracked.chat.completions.create({ model: 'hai-7b', messages: [] });

    expect(tracker.track).toHaveBeenCalledWith(
      expect.objectContaining({ customerId: 'hitachi_default' })
    );
  });
  ...
  it('strips llmeter_customer_id from options before passing to API', async () => {
    ...
    await tracked.chat.completions.create(
      { model: 'hai-70b-instruct', messages: [] },
