...
import { wrapGaiaNet } from './gaianet.js';
...
describe('wrapGaiaNet', () => {
  ...
  it('uses llmeter_customer_id from options when provided', async () => {
    ...
    await tracked.chat.completions.create(
      ...
      { llmeter_customer_id: 'gaianet_customer_123' } as never
    );

    expect(tracker.track).toHaveBeenCalledWith(
      expect.objectContaining({ customerId: 'gaianet_customer_123' })
    );
  });
  ...
  it('removes llmeter_customer_id from options before forwarding', async () => {
    ...
    await tracked.chat.completions.create(
      ...
      { llmeter_customer_id: 'wasm_dev_456', stream: false } as never
    );
  ...
  it('uses defaultCustomerId when llmeter_customer_id not set', async () => {
    ...
    expect(tracker.track).toHaveBeenCalledWith(
      expect.objectContaining({ customerId: 'gaianet_app' })
    );
  });

  it('passes through non-chat properties unchanged', async () => {
    const client = {
      ...makeMockClient({ model: 'phi-3-mini-4k-instruct' }),
      models: { list: vi.fn() },
    };
    const tracker = makeTracker();
    const tracked = wrapGaiaNet(client as never, tracker as never);
