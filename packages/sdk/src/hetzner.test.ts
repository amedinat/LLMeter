...
import { wrapHetzner } from './hetzner.js';
...
describe('wrapHetzner', () => {
  it('tracks usage when completion includes usage data', async () => {
    ...
    const tracked = wrapHetzner(client as never, tracker as never);
  ...
  it('uses llmeter_customer_id from options when provided', async () => {
    ...
    await tracked.chat.completions.create(
      ...
      { llmeter_customer_id: 'hetzner_customer_123' } as never
    );

    expect(tracker.track).toHaveBeenCalledWith(
      expect.objectContaining({ customerId: 'hetzner_customer_123' })
    );
  });
  ...
  it('removes llmeter_customer_id from options before forwarding', async () => {
    ...
    await tracked.chat.completions.create(
      ...
      { llmeter_customer_id: 'eu_dev_456', stream: false } as never
    );
  ...
  it('uses defaultCustomerId when llmeter_customer_id not set', async () => {
    ...
    const tracker = makeTracker();
    const tracked = wrapHetzner(client as never, tracker as never, 'hetzner_app');

    await tracked.chat.completions.create({ model: 'qwen2.5-72b-instruct', messages: [] });

    expect(tracker.track).toHaveBeenCalledWith(
      expect.objectContaining({ customerId: 'hetzner_app' })
    );
  });

  it('passes through non-chat properties unchanged', async () => {
    const client = {
      ...makeMockClient({ model: 'mixtral-8x7b-instruct' }),
      models: { list: vi.fn() },
    };
    const tracker = makeTracker();
    const tracked = wrapHetzner(client as never, tracker as never);
  ...
  it('forwards params correctly to underlying client', async () => {
    ...
    const params = {
      model: 'llama-3.3-70b-instruct',
      messages: [{ role: 'user', content: 'What is Hetzner?' }],
      temperature: 0.7,
    };
