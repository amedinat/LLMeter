...
import { wrapE2ENetworks } from './e2enetworks.js';
...
describe('wrapE2ENetworks', () => {
  it('tracks usage when completion includes usage data', async () => {
    ...
    const tracked = wrapE2ENetworks(client as never, tracker as never);
  ...
  it('uses llmeter_customer_id from options when provided', async () => {
    ...
    await tracked.chat.completions.create(
      ...
      { llmeter_customer_id: 'e2e_user_123' } as never
    );

    expect(tracker.track).toHaveBeenCalledWith(
      expect.objectContaining({ customerId: 'e2e_user_123' })
    );
  });
  ...
  it('removes llmeter_customer_id from options before forwarding', async () => {
    ...
    await tracked.chat.completions.create(
      ...
      { llmeter_customer_id: 'india_dev_456', stream: false } as never
    );
  ...
  it('uses defaultCustomerId when llmeter_customer_id not set', async () => {
    ...
    const tracker = makeTracker();
    const tracked = wrapE2ENetworks(client as never, tracker as never, 'e2e_app');

    await tracked.chat.completions.create({ model: 'Qwen/Qwen2.5-72B-Instruct', messages: [] });

    expect(tracker.track).toHaveBeenCalledWith(
      expect.objectContaining({ customerId: 'e2e_app' })
    );
  });
