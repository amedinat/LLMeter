...
import { wrapLightningAI } from './lightningai.js';
...
describe('wrapLightningAI', () => {
  it('tracks usage when completion includes usage data', async () => {
    ...
    const tracked = wrapLightningAI(client as never, tracker as never);
  ...
  it('uses llmeter_customer_id from options when provided', async () => {
    ...
    await tracked.chat.completions.create(
      ...
      { llmeter_customer_id: 'lightning_customer_123' } as never
    );

    expect(tracker.track).toHaveBeenCalledWith(
      expect.objectContaining({ customerId: 'lightning_customer_123' })
    );
  });
  ...
  it('removes llmeter_customer_id from options before forwarding', async () => {
    ...
    await tracked.chat.completions.create(
      ...
      { llmeter_customer_id: 'pytorch_dev_456', stream: false } as never
    );
  ...
  it('uses defaultCustomerId when llmeter_customer_id not set', async () => {
    ...
    const tracker = makeTracker();
    const tracked = wrapLightningAI(client as never, tracker as never, 'lightning_app');

    await tracked.chat.completions.create({ model: 'meta-llama/Llama-3.1-8B-Instruct', messages: [] });

    expect(tracker.track).toHaveBeenCalledWith(
      expect.objectContaining({ customerId: 'lightning_app' })
    );
  });

  it('passes through non-chat properties unchanged', async () => {
    const client = {
      ...makeMockClient({ model: 'microsoft/Phi-4' }),
      models: { list: vi.fn() },
    };
    const tracker = makeTracker();
    const tracked = wrapLightningAI(client as never, tracker as never);
