...
import { wrapAbacusAI } from './abacusai.js';
...
describe('wrapAbacusAI', () => {
  ...
  it('uses llmeter_customer_id from options when provided', async () => {
    ...
    await tracked.chat.completions.create(
      ...
      { llmeter_customer_id: 'fortune500_tenant_xyz' } as never
    );

    expect(tracker.track).toHaveBeenCalledWith(
      expect.objectContaining({ customerId: 'fortune500_tenant_xyz' })
    );
  });
  ...
  it('uses defaultCustomerId when llmeter_customer_id not set', async () => {
    ...
    expect(tracker.track).toHaveBeenCalledWith(
      expect.objectContaining({ customerId: 'acme_corp' })
    );
  });
