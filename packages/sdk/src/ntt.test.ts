...
describe('wrapNTT', () => {
  it('tracks usage when completion includes usage data', async () => {
    ...
    const tracked = wrapNTT(client as never, tracker as never);
  ...
  it('uses llmeter_customer_id from options when provided', async () => {
    ...
    await tracked.chat.completions.create(
      { model: 'tsuzumi-7b-v2', messages: [] },
      { llmeter_customer_id: 'ntt_user_123' } as never
    );

    expect(tracker.track).toHaveBeenCalledWith(
      expect.objectContaining({ customerId: 'ntt_user_123' })
    );
  });

  it('does not track when usage is missing', async () => {
    const client = makeMockClient({ model: 'tsuzumi-light' });
    const tracker = makeTracker();
    const tracked = wrapNTT(client as never, tracker as never);
  ...
  it('removes llmeter_customer_id from options before forwarding', async () => {
    ...
    await tracked.chat.completions.create(
      ...
      { llmeter_customer_id: 'japan_enterprise_456', stream: false } as never
    );
  ...
  it('uses defaultCustomerId when llmeter_customer_id not set', async () => {
    ...
    const tracker = makeTracker();
    const tracked = wrapNTT(client as never, tracker as never, 'ntt_app');

    await tracked.chat.completions.create({ model: 'tsuzumi-13b', messages: [] });

    expect(tracker.track).toHaveBeenCalledWith(
      expect.objectContaining({ customerId: 'ntt_app' })
    );
  });
