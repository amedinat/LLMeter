...
import { wrapDataCrunch } from './datacrunch.js';
...
describe('wrapDataCrunch', () => {
  it('tracks usage when completion includes usage data', async () => {
    ...
    const tracked = wrapDataCrunch(client as never, tracker as never);
  ...
  it('uses llmeter_customer_id from options when provided', async () => {
    ...
    await tracked.chat.completions.create(
      ...
      { llmeter_customer_id: 'datacrunch_user_123' } as never
    );

    expect(tracker.track).toHaveBeenCalledWith(
      expect.objectContaining({ customerId: 'datacrunch_user_123' })
    );
  });
  ...
  it('uses defaultCustomerId when llmeter_customer_id not set', async () => {
    ...
    const tracker = makeTracker();
    const tracked = wrapDataCrunch(client as never, tracker as never, 'datacrunch_app');

    await tracked.chat.completions.create({ model: 'llama-3.3-70b-instruct', messages: [] });

    expect(tracker.track).toHaveBeenCalledWith(
      expect.objectContaining({ customerId: 'datacrunch_app' })
    );
  });
