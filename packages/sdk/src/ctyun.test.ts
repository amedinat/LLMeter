...
import { wrapCTyun } from './ctyun.js';
...
describe('wrapCTyun', () => {
  it('tracks usage when completion includes usage data', async () => {
    ...
    const tracked = wrapCTyun(client as never, tracker as never);
  ...
  it('uses llmeter_customer_id from options when provided', async () => {
    ...
    await tracked.chat.completions.create(
      { model: 'telechat2-35b', messages: [] },
      { llmeter_customer_id: 'ctyun_user_456' } as never
    );

    expect(tracker.track).toHaveBeenCalledWith(
      expect.objectContaining({ customerId: 'ctyun_user_456' })
    );
  });
  ...
  it('strips llmeter_customer_id from options passed to the provider', async () => {
    ...
    await tracked.chat.completions.create(
      ...
      { llmeter_customer_id: 'ctyun_enterprise_789', stream: false } as never
    );
  ...
  it('falls back to defaultCustomerId when no llmeter_customer_id in options', async () => {
    ...
    const tracker = makeTracker();
    const tracked = wrapCTyun(
      client as never,
      tracker as never,
      'default-ctyun-tenant'
    );

    await tracked.chat.completions.create({ model: 'telechat-12b', messages: [] });

    expect(tracker.track).toHaveBeenCalledWith(
      expect.objectContaining({ customerId: 'default-ctyun-tenant' })
    );
  });
  ...
  it('returns the completion result unchanged', async () => {
    const response = {
      ...
      choices: [{ message: { role: 'assistant', content: '天翼云 TeleChat response' } }],
    };
