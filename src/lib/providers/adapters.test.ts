...
describe('Provider Adapters', () => {
  ...
  describe('gmiAdapter', () => {
    it('validates a correct GMI Cloud API key', async () => {
      fetchMock.mockResolvedValueOnce({ ok: true, json: async () => ({ data: [] }) });

      const result = await gmiAdapter.validateKey('gmi-test-api-key');
      ...
      expect(fetchMock).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          headers: expect.objectContaining({
            Authorization: 'Bearer gmi-test-api-key',
          }),
        })
      );
    });
    ...
    it('throws on 401 with helpful message', async () => {
      ...
      await expect(gmiAdapter.validateKey('bad-key')).rejects.toThrow(
    ...
    it('throws with server error message on non-401 failure', async () => {
      ...
      await expect(gmiAdapter.validateKey('some-key')).rejects.toThrow(
        'Internal server error'
      );
    });

    it('throws when api key is empty', async () => {
      await expect(gmiAdapter.validateKey('')).rejects.toThrow(
        'GMI Cloud API key is missing. Get your key from console.gmicloud.ai.'
      );
    });

    it('fetchUsage returns empty array', async () => {
      const records = await gmiAdapter.fetchUsage(
        'gmi-test-key',
        new Date('2026-05-01'),
        new Date('2026-05-29')
      );
    ...
    it('gmiAdapter.type is gmi', () => {
      expect(gmiAdapter.type).toBe('gmi');
    });
  ...
  describe('internlmAdapter', () => {
    it('validates a correct InternLM API key', async () => {
      fetchMock.mockResolvedValueOnce({ ok: true, json: async () => ({ data: [] }) });

      const result = await internlmAdapter.validateKey('internlm-test-api-key');
      ...
      expect(fetchMock).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          headers: expect.objectContaining({
            Authorization: 'Bearer internlm-test-api-key',
          }),
        })
      );
    });
    ...
    it('throws on 401 with helpful message', async () => {
      ...
      await expect(internlmAdapter.validateKey('bad-key')).rejects.toThrow(
    ...
    it('throws with server error message on non-401 failure', async () => {
      ...
      await expect(internlmAdapter.validateKey('some-key')).rejects.toThrow(
        'Internal server error'
      );
    });
    ...
    it('fetchUsage returns empty array', async () => {
      const records = await internlmAdapter.fetchUsage(
        'internlm-test-key',
        new Date('2026-05-01'),
        new Date('2026-05-29')
      );
    ...
    it('internlmAdapter.type is internlm', () => {
      expect(internlmAdapter.type).toBe('internlm');
    });
  ...
  describe('bentocloudAdapter', () => {
    it('validateKey returns true on 200 response', async () => {
      fetchMock.mockResolvedValueOnce({ ok: true, json: async () => ({ data: [{ id: 'llama-3.3-70b-instruct' }] }) });

      const result = await bentocloudAdapter.validateKey('test-bentocloud-api-key');
      expect(result).toBe(true);
      expect(fetchMock).toHaveBeenCalledWith(
        'https://api.cloud.bentoml.com/v1/models',
        expect.objectContaining({
          headers: { Authorization: 'Bearer test-bentocloud-api-key' },
        })
      );
    });

    it('throws on 401 with \'Invalid BentoCloud API key. Get your key from cloud.bentoml.com/api_tokens.\'', async () => {
      fetchMock.mockResolvedValueOnce({
        ok: false,
        status: 401,
        json: async () => ({ error: { message: 'Unauthorized' } }),
      ...
      await expect(bentocloudAdapter.validateKey('bad-key')).rejects.toThrow(
    ...
    it('throws on non-401 error with API message', async () => {
      ...
      await expect(bentocloudAdapter.validateKey('some-key')).rejects.toThrow(
        'Internal Server Error'
      );
    });

    it('throws when api key is empty with \'BentoCloud API key is missing. Get your key from cloud.bentoml.com/api_tokens.\'', async () => {
      await expect(bentocloudAdapter.validateKey('')).rejects.toThrow(
        'BentoCloud API key is missing. Get your key from cloud.bentoml.com/api_tokens.'
      );
    });

    it('fetchUsage returns empty array', async () => {
      const records = await bentocloudAdapter.fetchUsage(
        'test-key',
        new Date('2026-06-01'),
        new Date('2026-06-30')
      );
    ...
    it('sends Bearer token in Authorization header', async () => {
      ...
      await bentocloudAdapter.validateKey('my-bentocloud-key-xyz');

      expect(fetchMock).toHaveBeenCalledWith(
        expect.any(String),
        expect.objectContaining({
          headers: {
            Authorization: 'Bearer my-bentocloud-key-xyz',
          },
        })
      );
    });
  ...
  describe('hetznerAdapter', () => {
    it('validateKey returns true for a valid Hetzner API token', async () => {
      fetchMock.mockResolvedValueOnce({ ok: true, status: 200, json: async () => ({ object: 'list', data: [] }) });
      const result = await hetznerAdapter.validateKey('test-hetzner-api-token');
      expect(result).toBe(true);
      expect(fetchMock).toHaveBeenCalledWith(
        'https://inference.hetzner.cloud/v1/models',
        expect.objectContaining({
          headers: { Authorization: 'Bearer test-hetzner-api-token' },
        })
      );
    });

    it('throws for a 401 response with invalid token message', async () => {
      fetchMock.mockResolvedValueOnce({
        ok: false, status: 401, json: async () => ({ error: { message: 'Unauthorized' } }),
      });
    ...
    it('throws for a 500 response with server error message', async () => {
      ...
      await expect(hetznerAdapter.validateKey('some-token')).rejects.toThrow(
        'Internal Server Error'
      );
    });

    it('throws when api token is empty with missing token message', async () => {
      await expect(hetznerAdapter.validateKey('')).rejects.toThrow(
        'Hetzner API token is missing.'
      );
    });

    it('fetchUsage returns empty array', async () => {
      const records = await hetznerAdapter.fetchUsage(
        'test-token',
        new Date('2026-06-01'),
        new Date('2026-06-30')
      );
    ...
    it('hetznerAdapter.type is \'hetzner\'', () => {
      expect(hetznerAdapter.type).toBe('hetzner');
    });

    it('throws for a 403 response with forbidden token message', async () => {
      fetchMock.mockResolvedValueOnce({
        ok: false, status: 403, json: async () => ({ error: { message: 'Forbidden' } }),
      });
  ...
  describe('huaweiAdapter', () => {
    it('validates a correct API key', async () => {
      fetchMock.mockResolvedValueOnce({ ok: true, json: async () => ({ data: [] }) });
      const result = await huaweiAdapter.validateKey('test-huawei-api-key');
      expect(result).toBe(true);
      expect(fetchMock).toHaveBeenCalledWith(
        'https://api.modelfarm.cn/v1/models',
        expect.objectContaining({
          headers: { Authorization: 'Bearer test-huawei-api-key' },
        })
      );
    });
    ...
    it('throws on 401 with helpful message', async () => {
      ...
      await expect(huaweiAdapter.validateKey('bad-key')).rejects.toThrow(
        'Invalid Huawei Cloud API key.'
      );
    });
    ...
    it('throws on non-401 error with API message', async () => {
      ...
      await expect(huaweiAdapter.validateKey('some-key')).rejects.toThrow(
        'Internal error'
      );
    });

    it('throws when API key is empty', async () => {
      await expect(huaweiAdapter.validateKey('')).rejects.toThrow(
        'Huawei Cloud API key is missing.'
      );
    });

    it('fetchUsage returns empty array', async () => {
      const records = await huaweiAdapter.fetchUsage(
        'any-key',
        new Date('2026-06-01'),
        new Date('2026-06-05')
      );
    ...
    it("huaweiAdapter.type is 'huawei'", () => {
      expect(huaweiAdapter.type).toBe('huawei');
    });
  ...
  describe('sealionAdapter', () => {
    it('validates a correct API key', async () => {
      fetchMock.mockResolvedValueOnce({ ok: true, json: async () => ({ data: [] }) });
      const result = await sealionAdapter.validateKey('test-sealion-api-key');
      expect(result).toBe(true);
      expect(fetchMock).toHaveBeenCalledWith(
        'https://api.sea-lion.ai/v1/models',
        expect.objectContaining({
          headers: { Authorization: 'Bearer test-sealion-api-key' },
        })
      );
    });
    ...
    it('throws on non-401 error with API message', async () => {
      ...
      await expect(sealionAdapter.validateKey('some-key')).rejects.toThrow(
        'Internal error'
      );
    });

    it('throws when API key is empty', async () => {
      await expect(sealionAdapter.validateKey('')).rejects.toThrow(
        'AI Singapore SEA-LION API key is missing.'
      );
    });

    it('fetchUsage returns empty array', async () => {
      const records = await sealionAdapter.fetchUsage(
        'any-key',
        new Date('2026-06-01'),
        new Date('2026-06-05')
      );
    ...
    it("sealionAdapter.type is 'sealion'", () => {
      expect(sealionAdapter.type).toBe('sealion');
    });
  ...
  describe('canonAdapter', () => {
    it('validates a good key via GET /v1/models', async () => {
      fetchMock.mockResolvedValueOnce({ ok: true, json: async () => ({ data: [] }) });
      const result = await canonAdapter.validateKey('test-canon-myriad-api-key');
      expect(result).toBe(true);
      expect(fetchMock).toHaveBeenCalledWith(
        'https://api.myriad.canon/v1/models',
        expect.objectContaining({
          headers: { Authorization: 'Bearer test-canon-myriad-api-key' },
        })
      );
    });
    ...
    it('throws on 401 with a helpful message', async () => {
      ...
      await expect(canonAdapter.validateKey('bad-key')).rejects.toThrow(
        'Invalid Canon MYRIAD AI API key.'
      );
    });
    ...
    it('throws on non-401 API error', async () => {
      ...
      await expect(canonAdapter.validateKey('some-key')).rejects.toThrow(
        'Server error'
      );
    });

    it('throws when key is empty', async () => {
      await expect(canonAdapter.validateKey('')).rejects.toThrow(
        'Canon MYRIAD AI API key is missing.'
      );
    });

    it('fetchUsage returns an empty array', async () => {
      const records = await canonAdapter.fetchUsage(
        'test-key',
        new Date('2026-06-01'),
        new Date('2026-06-07')
      );
    ...
    it("canonAdapter.type is 'canon'", () => {
      expect(canonAdapter.type).toBe('canon');
    });
  ...
  describe('mitsubishiAdapter', () => {
    it('validates a good key via GET /v1/models', async () => {
      fetchMock.mockResolvedValueOnce({ ok: true, json: async () => ({ data: [] }) });
      const result = await mitsubishiAdapter.validateKey('test-mitsubishi-maisart-api-key');
      expect(result).toBe(true);
      expect(fetchMock).toHaveBeenCalledWith(
        'https://api.maisart.mitsubishielectric.com/v1/models',
        expect.objectContaining({
          headers: { Authorization: 'Bearer test-mitsubishi-maisart-api-key' },
        })
      );
    });
    ...
    it('throws on 401 with a helpful message', async () => {
      ...
      await expect(mitsubishiAdapter.validateKey('bad-key')).rejects.toThrow(
        'Invalid Mitsubishi Electric MAISART AI API key.'
      );
    });
    ...
    it('throws on non-401 API error', async () => {
      ...
      await expect(mitsubishiAdapter.validateKey('some-key')).rejects.toThrow(
        'Server error'
      );
    });

    it('throws when key is empty', async () => {
      await expect(mitsubishiAdapter.validateKey('')).rejects.toThrow(
        'Mitsubishi Electric MAISART AI API key is missing.'
      );
    });

    it('fetchUsage returns an empty array', async () => {
      const records = await mitsubishiAdapter.fetchUsage(
        'test-key',
        new Date('2026-06-01'),
        new Date('2026-06-07')
      );
    ...
    it("mitsubishiAdapter.type is 'mitsubishi'", () => {
      expect(mitsubishiAdapter.type).toBe('mitsubishi');
    });
  ...
  describe('toshibaAdapter', () => {
    it('validates a good key via GET /v1/models', async () => {
      fetchMock.mockResolvedValueOnce({ ok: true, json: async () => ({ data: [] }) });
      const result = await toshibaAdapter.validateKey('test-toshiba-tbrain-api-key');
      expect(result).toBe(true);
      expect(fetchMock).toHaveBeenCalledWith(
        'https://api.t-brain.toshiba.com/v1/models',
        expect.objectContaining({
          headers: { Authorization: 'Bearer test-toshiba-tbrain-api-key' },
        })
      );
    });
    ...
    it('throws on non-401 API error', async () => {
      ...
      await expect(toshibaAdapter.validateKey('some-key')).rejects.toThrow(
        'Server error'
      );
    });

    it('throws when key is empty', async () => {
      await expect(toshibaAdapter.validateKey('')).rejects.toThrow(
        'Toshiba T-Brain AI API key is missing.'
      );
    });

    it('fetchUsage returns an empty array', async () => {
      const records = await toshibaAdapter.fetchUsage(
        'test-key',
        new Date('2026-06-01'),
        new Date('2026-06-07')
      );
    ...
    it("toshibaAdapter.type is 'toshiba'", () => {
      expect(toshibaAdapter.type).toBe('toshiba');
    });
