...
export const metadata: Metadata = {
  title: 'LiteLLM vs LLMeter — Per-Customer LLM Cost Without Running a Proxy',
  description:
    'LiteLLM is an open-source Python proxy you self-host (proxy + Postgres + Redis) for per-key spend. LLMeter gives per-customer cost and margin with no proxy and no infra. Open-source comparison.',
  metadataBase: new URL('https://www.llmeter.org'),
  openGraph: {
    title: 'LiteLLM vs LLMeter — per-customer cost without a proxy',
    description:
      'LiteLLM is a self-hosted Python proxy (proxy + Postgres + Redis). LLMeter gives per-customer cost and margin from a read-only key — no proxy, no infra. Open-source.',
    url: 'https://www.llmeter.org/migrate/litellm',
  ...
  twitter: {
    ...
    title: 'LiteLLM vs LLMeter — per-customer cost without a proxy',
    description:
      'LLMeter tracks per-customer LLM cost and margin without a proxy or infra to run. Read-only API keys. Free for 1 provider.',
  },
  alternates: {
    canonical: 'https://www.llmeter.org/migrate/litellm',
  },
};

const COMPARISON = [
  { feature: 'Setup method', litellm: 'Python proxy (self-host + route traffic)', llmeter: 'Read-only API key + optional SDK tag' },
  { feature: 'Sits in request path', litellm: 'Yes — proxy server', llmeter: 'No' },
  { feature: 'Added latency', litellm: 'Python runtime overhead at scale', llmeter: 'Zero' },
  { feature: 'Infra to run and maintain', litellm: 'Proxy + PostgreSQL + Redis', llmeter: 'None' },
  { feature: 'Attribution depth', litellm: 'Per-key / user / team (flatter)', llmeter: 'Per-customer + feature + environment + margin' },
  { feature: 'Cost vs revenue (margin)', litellm: 'No', llmeter: 'Yes' },
  { feature: 'Open source', litellm: 'Yes', llmeter: 'Yes' },
  { feature: 'Prompt/data access', litellm: 'Proxies all traffic', llmeter: 'Never sees prompts or completions' },
  { feature: 'Pricing', litellm: 'Open source + enterprise', llmeter: 'Free / $19 / $49' },
];
...
const MIGRATION_STEPS = [
  ...
  {
    step: 3,
    title: 'Keep LiteLLM for routing if you use it',
    description: 'Keep LiteLLM for routing/fallback if you use it — LLMeter does not replace your gateway, it adds per-customer cost and margin on top, with no extra infra.',
    icon: RefreshCw,
  },
];
...
const FAQ_ITEMS = [
  {
    question: 'Do I have to give up LiteLLM to use LLMeter?',
    answer:
  ...
  {
    question: 'Does LLMeter need a proxy, Postgres, and Redis like LiteLLM?',
    answer:
...
export default function MigrateLiteLLM() {
  return (
      ...
      <main className="flex-1">
        ...
        <section className="space-y-6 pb-8 pt-6 md:pb-12 md:pt-10 lg:py-32">
          <div className="container flex max-w-[64rem] flex-col items-center gap-4 text-center">
            <Badge variant="secondary" className="rounded-2xl px-4 py-1.5 text-sm">
              LiteLLM &rarr; LLMeter Migration Guide
            </Badge>
            <h1 className="text-4xl font-bold leading-[1.1] tracking-tight sm:text-5xl md:text-6xl lg:text-7xl">
              ...
              <span className="bg-gradient-to-r from-primary to-cyan-300 bg-clip-text text-transparent">
                without running a proxy
              </span>
            </h1>
            <p className="max-w-[42rem] leading-normal text-muted-foreground sm:text-xl sm:leading-8">
              LiteLLM is an open-source Python proxy you self-host — proxy plus PostgreSQL plus Redis — to track per-key spend.
              LLMeter gives you deeper per-customer cost and margin from a read-only API key. No proxy. No database. No infra to run.
            </p>
        ...
        <section className="container py-12 md:py-20">
          <div className="mx-auto max-w-[64rem]">
            <h2 className="text-center text-3xl font-bold md:text-4xl mb-4">Why look past the proxy?</h2>
            ...
            <div className="grid gap-6 md:grid-cols-3">
              {[
                {
                  icon: Clock,
                  title: 'No infra to run',
                  description: 'Spend tracking on LiteLLM means a proxy plus PostgreSQL plus Redis to deploy and keep healthy. LLMeter has nothing for you to run — connect a key.',
                },
                {
                  icon: Shield,
                  title: 'Out of your request path',
                  description: 'A Python proxy sits in front of every call, adding runtime overhead at scale and a failure mode. LLMeter reads usage out-of-band — zero latency.',
                },
                {
                  icon: Zap,
                  title: 'Deeper attribution + margin',
                  description: 'LiteLLM attributes per key, user, or team. LLMeter goes per-customer, feature, and environment — and adds cost-vs-revenue margin.',
                },
              ].map((item) => (
        ...
        <section id="comparison" className="container py-12 md:py-20 border-t">
          <div className="mx-auto max-w-[64rem]">
            <h2 className="text-center text-3xl font-bold md:text-4xl mb-4">LiteLLM vs LLMeter</h2>
            <p className="text-center text-muted-foreground text-lg max-w-[42rem] mx-auto mb-12">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b bg-muted/30">
                    ...
                    <th className="px-4 py-3 text-left font-semibold text-muted-foreground">LiteLLM</th>
                ...
                <tbody>
                  {COMPARISON.map((row) => (
                    <tr key={row.feature} className="border-b last:border-0">
                      ...
                      <td className="px-4 py-3 text-muted-foreground">{row.litellm}</td>
                      <td className="px-4 py-3">
                        {row.llmeter === 'Zero' || row.llmeter === 'None' || row.llmeter === 'No' || row.llmeter === 'Per-customer + feature + environment + margin' ? (
        ...
        <section className="container py-12 md:py-20 border-t">
          <div className="mx-auto max-w-[64rem]">
            <h2 className="text-center text-3xl font-bold md:text-4xl mb-4">No proxy. No database to babysit.</h2>
            <p className="text-center text-muted-foreground text-lg max-w-[42rem] mx-auto mb-12">
            ...
            <div className="grid gap-8 md:grid-cols-2">
              <div className="glass-card p-6 space-y-4 border-destructive/20">
                <h3 className="text-lg font-semibold flex items-center gap-2">
                  <X className="h-5 w-5 text-destructive" /> LiteLLM (proxy-based)
                </h3>
                <ul className="space-y-2 text-sm text-muted-foreground">
                  <li className="flex items-start gap-2"><X className="h-4 w-4 text-destructive shrink-0 mt-0.5" /> All LLM traffic routes through the proxy server</li>
                  <li className="flex items-start gap-2"><X className="h-4 w-4 text-destructive shrink-0 mt-0.5" /> Python runtime overhead on every call at scale</li>
                  <li className="flex items-start gap-2"><X className="h-4 w-4 text-destructive shrink-0 mt-0.5" /> Requires proxy + PostgreSQL + Redis to run</li>
              ...
              <div className="glass-card p-6 space-y-4 border-primary/20">
                ...
                <ul className="space-y-2 text-sm text-muted-foreground">
                  ...
                  <li className="flex items-start gap-2"><Check className="h-4 w-4 text-primary shrink-0 mt-0.5" /> No proxy, no database, no Redis — connect a key</li>
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              itemListElement: [
                ...
                { '@type': 'ListItem', position: 2, name: 'Migrate from LiteLLM', item: 'https://www.llmeter.org/migrate/litellm' },
              ],
            }),
          }}
        />
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              '@context': 'https://schema.org',
              '@type': 'WebPage',
              name: 'LiteLLM vs LLMeter — Per-Customer LLM Cost Without Running a Proxy',
              description: 'Side-by-side guide for teams comparing LiteLLM (self-hosted Python proxy) to LLMeter (read-only API key, per-customer cost and margin).',
              url: 'https://www.llmeter.org/migrate/litellm',
              mainEntity: {
                '@type': 'FAQPage',
