...
export const metadata: Metadata = {
  title: 'Bifrost vs LLMeter — Per-Customer LLM Cost Tracking Without a Gateway',
  description:
    'Bifrost (Maxim AI) tracks per-customer LLM spend as an AI gateway in your request path. LLMeter gives you per-customer cost plus unit-economics margin without running a gateway or adding latency. Open-source comparison.',
  metadataBase: new URL('https://www.llmeter.org'),
  openGraph: {
    title: 'Bifrost vs LLMeter — per-customer cost without a gateway',
    description:
      'Bifrost is an AI gateway in your request path. LLMeter gives per-customer cost plus margin from a read-only key — no gateway, no latency. Open-source.',
    url: 'https://www.llmeter.org/migrate/bifrost',
  ...
  twitter: {
    ...
    title: 'Bifrost vs LLMeter — per-customer cost without a gateway',
    description:
      'LLMeter tracks per-customer LLM cost and margin without sitting in your request path. Read-only API keys. Free for 1 provider.',
  },
  alternates: {
    canonical: 'https://www.llmeter.org/migrate/bifrost',
  },
};

const COMPARISON = [
  { feature: 'Setup method', bifrost: 'AI gateway (route traffic through Bifrost)', llmeter: 'Read-only API key + optional SDK tag (no gateway)' },
  { feature: 'Sits in request path', bifrost: 'Yes — all LLM calls route through it', llmeter: 'No — reads usage out-of-band' },
  { feature: 'Added latency', bifrost: 'Gateway hop (must run the gateway)', llmeter: 'Zero (never in your request path)' },
  { feature: 'Per-customer attribution', bifrost: 'Yes (virtual keys, 4-tier budgets)', llmeter: 'Yes (customer_id via SDK / ingest)' },
  { feature: 'Cost vs revenue (margin)', bifrost: 'No', llmeter: 'Yes — per-customer unit economics' },
  { feature: 'Infra to run and maintain', bifrost: 'Yes — self-host the gateway', llmeter: 'None — connect a key' },
  { feature: 'Open source', bifrost: 'Yes (Apache 2.0)', llmeter: 'Yes' },
  { feature: 'Prompt/data access', bifrost: 'Proxies all traffic', llmeter: 'Never sees prompts or completions' },
  { feature: 'Pricing', bifrost: 'Open source + enterprise (contact sales)', llmeter: 'Free / $19 / $49' },
];
...
const MIGRATION_STEPS = [
  ...
  {
    step: 3,
    title: 'Run LLMeter alongside Bifrost',
    description: 'Keep Bifrost in place if you want gateway enforcement — LLMeter reads usage out-of-band, so it runs alongside without touching your request path.',
    icon: RefreshCw,
  },
];
...
const FAQ_ITEMS = [
  {
    question: 'Does LLMeter sit in my request path like Bifrost does?',
    answer:
  ...
  {
    question: 'What does LLMeter add that Bifrost\'s budgets don\'t?',
    answer:
...
export default function MigrateBifrost() {
  return (
      ...
      <main className="flex-1">
        ...
        <section className="space-y-6 pb-8 pt-6 md:pb-12 md:pt-10 lg:py-32">
          <div className="container flex max-w-[64rem] flex-col items-center gap-4 text-center">
            <Badge variant="secondary" className="rounded-2xl px-4 py-1.5 text-sm">
              Bifrost &rarr; LLMeter Migration Guide
            </Badge>
            ...
            <p className="max-w-[42rem] leading-normal text-muted-foreground sm:text-xl sm:leading-8">
              Bifrost by Maxim AI is an AI gateway — every LLM call routes through it to enforce virtual keys and budgets.
              LLMeter gives you the same per-customer view, plus cost-vs-revenue margin, from a read-only API key. No gateway, no latency hop, no infra to run.
            </p>
        ...
        <section className="container py-12 md:py-20">
          <div className="mx-auto max-w-[64rem]">
            <h2 className="text-center text-3xl font-bold md:text-4xl mb-4">Why look past the gateway?</h2>
            ...
            <div className="grid gap-6 md:grid-cols-3">
              {[
                {
                  icon: Clock,
                  title: 'No gateway to run',
                  description: 'Bifrost is self-hosted infrastructure on the hot path. LLMeter reads usage out-of-band — there is nothing to deploy, scale, or keep up.',
                },
                {
                  icon: Shield,
                  title: 'Out of your request path',
                  description: 'A gateway is one more hop on every LLM call and one more thing that can fail. LLMeter never touches traffic, so it cannot add latency or break a call.',
                },
                {
                  icon: Zap,
                  title: 'Cost plus margin',
                  description: 'Bifrost gives per-customer budgets. LLMeter adds the revenue side — per-customer unit economics, so you see margin, not just spend.',
                },
              ].map((item) => (
        ...
        <section id="comparison" className="container py-12 md:py-20 border-t">
          <div className="mx-auto max-w-[64rem]">
            <h2 className="text-center text-3xl font-bold md:text-4xl mb-4">Bifrost vs LLMeter</h2>
            <p className="text-center text-muted-foreground text-lg max-w-[42rem] mx-auto mb-12">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b bg-muted/30">
                    ...
                    <th className="px-4 py-3 text-left font-semibold text-muted-foreground">Bifrost</th>
                ...
                <tbody>
                  {COMPARISON.map((row) => (
                    <tr key={row.feature} className="border-b last:border-0">
                      ...
                      <td className="px-4 py-3 text-muted-foreground">{row.bifrost}</td>
                      <td className="px-4 py-3">
                        {row.llmeter === 'Zero (never in your request path)' || row.llmeter === 'None — connect a key' || row.llmeter === 'No — reads usage out-of-band' || row.llmeter === 'Yes — per-customer unit economics' ? (
        ...
        <section className="container py-12 md:py-20 border-t">
          <div className="mx-auto max-w-[64rem]">
            <h2 className="text-center text-3xl font-bold md:text-4xl mb-4">No gateway. No request-path risk.</h2>
            <p className="text-center text-muted-foreground text-lg max-w-[42rem] mx-auto mb-12">
            ...
            <div className="grid gap-8 md:grid-cols-2">
              <div className="glass-card p-6 space-y-4 border-destructive/20">
                <h3 className="text-lg font-semibold flex items-center gap-2">
                  <X className="h-5 w-5 text-destructive" /> Bifrost (gateway-based)
                </h3>
                <ul className="space-y-2 text-sm text-muted-foreground">
                  <li className="flex items-start gap-2"><X className="h-4 w-4 text-destructive shrink-0 mt-0.5" /> All LLM traffic routes through the gateway</li>
                  <li className="flex items-start gap-2"><X className="h-4 w-4 text-destructive shrink-0 mt-0.5" /> A gateway hop adds latency to every call</li>
                  <li className="flex items-start gap-2"><X className="h-4 w-4 text-destructive shrink-0 mt-0.5" /> You self-host and maintain the infrastructure</li>
                  <li className="flex items-start gap-2"><X className="h-4 w-4 text-destructive shrink-0 mt-0.5" /> The gateway proxies all prompts and completions</li>
                </ul>
              </div>
              <div className="glass-card p-6 space-y-4 border-primary/20">
                ...
                <ul className="space-y-2 text-sm text-muted-foreground">
                  ...
                  <li className="flex items-start gap-2"><Check className="h-4 w-4 text-primary shrink-0 mt-0.5" /> No infra to run — connect a read-only key</li>
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              itemListElement: [
                ...
                { '@type': 'ListItem', position: 2, name: 'Migrate from Bifrost', item: 'https://www.llmeter.org/migrate/bifrost' },
              ],
            }),
          }}
        />
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              '@context': 'https://schema.org',
              '@type': 'WebPage',
              name: 'Bifrost vs LLMeter — Per-Customer LLM Cost Tracking Without a Gateway',
              description: 'Side-by-side guide for teams comparing Bifrost (AI gateway) to LLMeter (read-only API key, per-customer cost and margin).',
              url: 'https://www.llmeter.org/migrate/bifrost',
              mainEntity: {
                '@type': 'FAQPage',
