...
const ingestionSchema = z.array(usageEventSchema);
...
function calculateCost(event: z.infer<typeof usageEventSchema>): { cost: number; provider: string } {
  ...
  if (pricing) {
    ...
    // Fallback for unknown models - very rough estimate
    // A more robust solution would be to infer provider from model name prefix
    [inputRate, outputRate] = getDefaultRates('openai'); // Default to openai pricing as a fallback
    provider = 'unknown';
  }
...
export async function POST(req: NextRequest) {
  ...
  if (events.length === 0) {
    return NextResponse.json({ message: 'Received empty event list. Nothing to do.' }, { status: 200 });
  }
